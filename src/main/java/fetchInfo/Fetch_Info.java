...
public class Fetch_Info implements MessageCreateListener {
    ...
    public static String getInfo(String suffix) {
        try {
            ...
            Path fileName = Path.of(String.format("src/assets/info/info%s.txt", Integer.toString(test)));
            String content = Files.readString(fileName);
            return content;
        } catch (Exception e) {
            return("The number given isn't valid or the information doesn't exist");
        }
    }
    ...
    public void onMessageCreate(MessageCreateEvent event) {
        ...
        if(strArr.length>=3){
            if(strArr[0].equalsIgnoreCase("mc") && strArr[1].equalsIgnoreCase("info")){
                event.getChannel().sendMessage(getInfo(strArr[2]));
            }
        }
        if(strArr.length>=2){
            if(strArr[0].equalsIgnoreCase("mc") && strArr[1].equalsIgnoreCase("list")){
